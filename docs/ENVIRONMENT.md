...
| temporalio | 1.9.0 | MIT | Green | [PROD] Latest stable 1.x; async-first Python SDK |
| pytest | 8.2.0 | MIT | Green | [DEV] Dev dep; standard test runner |
| pytest-asyncio | 0.23.8 | MIT | Green | [DEV] Dev dep; async test support |

Docker images:

| Service | Image | License | Tier | Rationale |
|---------|-------|---------|------|-----------|
| Business Postgres | pgvector/pgvector:pg16 | MIT/PostgreSQL | Green | [PROD] pgvector built in; pg16 stable |
| Temporal Postgres | postgres:16.3 | PostgreSQL | Green | [PROD] Temporal's own event store; kept separate |
| Temporal server | temporalio/auto-setup:1.27.2 | MIT | Green | [PROD] Auto-inits DB schema; single-node dev |
| Temporal UI | temporalio/ui:2.31.2 | MIT | Green | [PROD] Web UI for workflow inspection — ships as the audit/report surface of the orchestration pod (Doc 11) |
| Ollama | ollama/ollama:0.6.5 | MIT | Green | **SUPERSEDED (2026-06): API-only inference.** No local inference pod ships; all LLM calls route via the infer contract → LiteLLM → configured external API provider. Row retained as a Phase A historical record; remove the service from the deploy bundle. |
| Harness base | python:3.12-slim | PSF | Green | [PROD] Matches Python pin; slim keeps image small |

[DEV] **Verification obligation:** Re-check all pins at integration time and on every upgrade
per doc 08 vetting workflow. License drift is real (n8n, Open WebUI precedent).

## Host assumptions

- [PROD] OS: Ubuntu (hardened per doc 05 / doc 11)
- [PROD] Container runtime: Podman (Apache 2.0 — green tier)
...
|---------|-----|---------|------|-------|
| asyncpg | 0.30.0 | MIT | Green | [PROD] Async Postgres driver; async activities only |
| hypothesis | >=6.100,<7 | MPL-2.0 | Yellow | [DEV] Dev-only; unmodified; not redistributed; property tests only |

[PROD] asyncpg: MIT license (not Apache-2.0 — both are green tier, constraint met).
[DEV] Hypothesis: MPL-2.0 is yellow tier per Doc 08. Obligation ("publish modifications
...
[PROD] Schema is applied once per fresh volume via the dedicated one-shot entrypoint:
```
python -m harness.shared.persistence.bootstrap
```
[PROD] This is NOT run inside the worker hot path (avoids multi-worker DDL races; works
...
[DEV] The dev overlay exposes `postgres-business` on `127.0.0.1:5433` for local testing
and bootstrap. Apply on top of canonical compose:
```
podman compose -f deploy/compose.yaml -f deploy/compose.dev.yaml up -d
```
[DEV] Port exposure is NOT in `compose.yaml` — keeps the canonical compose production-clean.
[DEV] `sslmode=disable` is acceptable for loopback dev.
`TODO(liquid: sslmode=require)` — when the business DB moves off-host (Phase E).

### Two-Postgres invariant

[PROD] The `build_dsn()` function (`harness/shared/persistence/dsn.py`) contains a
fail-fast guard that asserts `DB_HOST != postgres-temporal` and `POSTGRES_DB != temporal`.
[PROD] This is the only runtime enforcement of the two-DB hard rule. `DB_HOST=postgres-business`
